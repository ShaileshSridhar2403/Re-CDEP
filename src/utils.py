...
def load_precalculated_dataset(path):
    ...
    cancer_dataset = tf.data.Dataset.from_tensor_slices(
        (cancer_features, cancer_targets, -np.ones((len(cancer_features), 2, 25088))))
...
def get_auc_f1(model, dataset, fname=None, ):
    ...
    f1 = np.asarray(
        [f1_score(y_hat, y > x) for x in np.linspace(0.1, 1, num=10) if (y > x).any() and (y < x).any()]).max()
    return auc, f1
...
def load_img_dataset(path):
    ...
    num_test = num_total - num_train - num_val
    torch.manual_seed(0)
    train_dataset, test_dataset, val_dataset = torch.utils.data.random_split(complete_dataset,

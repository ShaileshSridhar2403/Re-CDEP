...
seg_path  = os.path.join(data_path, "segmentation")
not_cancer_path = os.path.join(data_path, "processed/0_no_cancer")
cancer_path = os.path.join(data_path, "processed/1_cancer")
...
# torch.manual_seed(args.seed); LOOK INTO THIS
...
#model.classifier[-1] = nn.Linear(4096, 2) #check if 4096 is just prev layer output or something more
...
def load_folder(path):
    ...
    for i in tqdm(range(num_files)):
        try:
            img = Image.open(os.path.join(path, list_files[i]))
...
def load_seg(path, orig_path):
    ...
    for i in tqdm(range(num_files)):
        if os.path.isfile(os.path.join(path,  list_files[i])):
            img = Image.open(os.path.join(path, list_files[i]))
...
not_cancer_dataset = tf.data.Dataset.from_tensor_slices((not_cancer_set.swapaxes(1,3).swapaxes(2,3),not_cancer_targets,seg_set))
del not_cancer_set
del seg_set
...
cancer_dataset = tf.data.Dataset.from_tensor_slices((cancer_set.swapaxes(1,3).swapaxes(2,2),cancer_targets,np.zeros((len(cancer_set), 299, 299))))
del cancer_set
...
torch.manual_seed(0); #Find out way to apply to tensorflow
#train_dataset, test_dataset, val_dataset= torch.utils.data.random_split(complete_dataset, [num_train, num_test, num_val])
#FIGURE OUT WAY TO MAKE RANDOM
train_dataset,test_dataset,val_dataset = tf.split(complete_dataset, [num_train, num_test, num_val])


datasets = {'train' : train_dataset.batch(args.batch_size), 'test':test_dataset.batch(args.batch_size), 'val': val_dataset.batch(args.batch_size)}
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                # model.train()  # Set model to training mode ECESSARY FOR KERAS
            else:
            ...
            if phase == 'val':
                if epoch_loss < best_loss:
                    ...
                    cur_patience+=1
        if cur_patience >= patience:  #if epoch loss is not better than best loss for n='patience' epochs, stop training
            break
...
torch.save(model.classifier.state_dict(),os.path.join(model_path, pid + ".pt"))

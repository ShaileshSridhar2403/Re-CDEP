        ...
        with tf.GradientTape() as tape:
            ...
            if regularizer_rate > 0:
                ...
                if mask.any():
                    ...
                    cur_cd_loss = tf.nn.functional.softmax(
                        tf.stack((tf.boolean_mask(rel[:, 0], mask)), tf.boolean_mask(irrel[:, 0], mask), axis=1))
                    cur_cd_loss = tf.nn.functional.softmax(
                        tf.stack((tf.boolean_mask(irrel[:, 1], mask)), tf.boolean_mask(rel[:, 0], mask), axis=1))
...
def create_classification_model():
    ...
    output_layer = tf.keras.layers.Dense(1,
                                         activation='sigmoid')  # check if 4096 is just prev layer output or something more
...
datasets, weights = utils.load_precalculated_dataset(
    dataset_path)  # NOTE: Not fully implemented, need to execute code to understand. REVISIT ALL DATASET/DATA LOADING CODE
...
print(classification_report(val_y,
                            val_predictions, target_names=["Cancer", "Not Cancer"]))

print("Printing Test Scores.........")
print(classification_report(test_y,
                            test_predictions, target_names=["Cancer", "Not Cancer"]))
